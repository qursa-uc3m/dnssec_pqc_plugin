...
type DNSKEY struct {
	...
	s       crypto.Signer
	tag     uint16
	privRaw []byte // Store the private key in raw format
}
...
// ParseKeyFile reads a DNSSEC keyfile as generated by dnssec-keygen or other utilities.
// It adds ".key" for the public key and ".private" for the private key.
func ParseKeyFile(pubFile, privFile string) (*DNSKEY, error) {
	...
	privRaw, err := os.ReadFile(privFile) // Store the private key in raw format
	if err != nil {
		return nil, err
	}

	if s, ok := p.(*rsa.PrivateKey); ok {
		return &DNSKEY{K: dk, D: dk.ToDS(dns.SHA256), s: s, tag: dk.KeyTag(), privRaw: privRaw}, nil
	}
	...
	if s, ok := p.([]byte); ok {
		// p es la clave privada de algoritmopqc en formato []byte.
		return &DNSKEY{
			K:       dk,
			D:       dk.ToDS(dns.SHA256),
			s:       nil, // No tenemos un crypto.Signer para algpqc
			tag:     dk.KeyTag(),
			privRaw: s,
		}, nil
	}
	return &DNSKEY{K: dk, D: dk.ToDS(dns.SHA256), s: nil, tag: 0, privRaw: privRaw}, errors.New("no private key found")
}
...
func ParseKeyFromAWSSecretsManager(secretID string) (*DNSKEY, error) {
	...
	// Store the private key in raw format
	privRaw := []byte(secretData.Private)
	...
	return &DNSKEY{K: dk, D: dk.ToDS(dns.SHA256), s: s, tag: tag, privRaw: privRaw}, nil
}
