...
func TestSetupDnssec(t *testing.T) {
	...
	tests := []struct {
		...
		{`dnssec_pqc`, false, nil, nil, false, defaultCap, ""},
		{`dnssec_pqc example.org`, false, []string{"example.org."}, nil, false, defaultCap, ""},
		{`dnssec_pqc 10.0.0.0/8`, false, []string{"10.in-addr.arpa."}, nil, false, defaultCap, ""},
		{
			`dnssec_pqc example.org {
				cache_capacity 100
			}`, false, []string{"example.org."}, nil, false, 100, "",
		},
		{
			`dnssec_pqc cluster.local {
				key file Kcluster.local
			}`, false, []string{"cluster.local."}, nil, false, defaultCap, "",
		},
		{
			`dnssec_pqc example.org cluster.local {
				key file Kcluster.local
		...
		{`dnssec_pqc
		  dnssec_pqc`, true, nil, nil, false, defaultCap, ""},
		{
			`dnssec_pqc cluster.local {
				key file Kcluster.local
